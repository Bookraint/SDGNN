...
for seed in 0; do
  python3 -u qagnn.py --dataset $dataset \
      --encoder $model --encoder_fix $model_fix -k $k --gnn_dim $gnndim -elr $elr -dlr $dlr -bs $bs -mbs $mbs --fp16 true --seed $seed --max_seq_len $max_seq_len --lr_schedule $lr_schedule --freeze_ent_emb $freeze_ent_emb \
